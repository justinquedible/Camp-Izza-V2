...
export const Attendance: React.FC<AttendanceProps> = () => {
    ...
    React.useEffect(() => {
        let foundGroup = AuthService.currentUser().group
        let testGroup = "coconuts1"

        let str = "showGroups." + testGroup + " = true"
    ...
    const handleDisplayGroup = (group: string) => {
        switch (group) {
            case "coconuts1":
                return (showGroups.coconuts1);
            case "coconuts2":
                return (showGroups.coconuts2);
            case "trees1":
                return (showGroups.trees1);
            case "trees2":
                return (showGroups.trees2);
            case "leaders1":
                return (showGroups.leaders1);
            case "leaders2":
                return (showGroups.leaders2);
            case "dates1":
                return (showGroups.dates1);
            case "dates2":
                return (showGroups.dates2);
        }
        return false;
    }
        ...
        return (<Modal {...props}
                ...
                <Modal.Body>
                    {campersInYear.map((item) =>
                        ...
                        //         <Tab.Content>
                        //             <Tab.Pane eventKey={item.id.toString()}>
                        //                 Are you sure you want to add {mergeCamper.camperName} to {item.firstName} {item.lastName}?
                        //             </Tab.Pane>
                        //             <Tab.Pane>
                        //                 <Button onClick={() =>{updateAttendanceRecord(item.id.toString(), mergeCamper.time); updateBonusAttendanceRecord(mergeCamper.camperName, "absent", mergeCamper.group)}}>Save</Button>
                        //             </Tab.Pane>
                        //         </Tab.Content>
                        //     </Col>
                        // </Tab.Container>
    ...
    return (
        ...
        <Container className="Admin-Buttons">
            <Button variant="primary" className="backButton" href="/#/counselor"> Back </Button><br/><br/>
            <h3> Attendance </h3>
            <form onSubmit={handleDateSubmit}>
                ...
                <div className={"buttonToggle"}><input type={"date"} className={"oneRow"}
                    ...
                    <Button variant={"success"} type={"submit"}> Go </Button></div>
            </form>
            <br/>
            <h6> -------------------- </h6><br/>
            ...
            <br/>
            <h6> Campers Present: {dateTimeValue.date == undefined ? "NA/NA" : getNumItems()}</h6>
