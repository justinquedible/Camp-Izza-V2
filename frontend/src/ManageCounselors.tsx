...
export default function ManageCounselors() {
    ...
    React.useEffect(() => {
        (async () => {
            await axios
              .get(process.env.REACT_APP_API + "api/counselors/getCounselors")
              .then(async (response) => {
    ...
    const handleApproveChange = async (id:string) => {
        for (let c of counselors){
            if (c.id === id){
                c.approved = true;
                c.active = true;
                await axios.put(process.env.REACT_APP_API + "api/counselors/updateCounselor/" + c.id, {...c, dob: dateTimeToDateInput(c.dob)})
            }
        }
