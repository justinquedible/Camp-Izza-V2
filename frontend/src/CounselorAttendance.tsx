...
export const CounselorAttendance: React.FC<AdminAttendanceProps> = () => {
  ...
  React.useEffect(() => {
    renderTermDropdown();
    renderWeekDropdown();
    // TODO: Get all campers in a group
    AdminService.getAllCounselors().then((response) => {
      if (response.status === 200) {
        ...
        if (counselors.length > 0) {
          ...
          cell.innerHTML = "<p>PU <Button>Reset</Button></p>";
          dates.reverse();
          for (let d of dates) {
          ...
          for (let c of counselors) {
            ...
            row.insertCell(0).innerHTML = "<div> <input type='checkbox'>" + c.firstName + " " + c.lastName + "</div>";
